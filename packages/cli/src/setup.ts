...
class SetupAcknowledgementUncertainError extends Error {
	constructor(cause: unknown) {
		super(
			"Auto setup acknowledgement completion is uncertain; the verified local profile was preserved. Re-run setup only after checking the profile in Auto.",
			{ cause },
		);
...
function profileMatchesRequest(
	profile: SetupProfile,
	request: {
		preset: McpSurface | "custom";
		accessPreset?: AccessPreset;
		categories?: string[];
	},
): boolean {
	const expectedAccess =
		request.preset === "research"
			? "read"
...
async function fetchSetupJson<T>(
	...
	const timeoutMs = Number.parseInt(
		process.env.AUTO_MCP_SETUP_REQUEST_TIMEOUT_MS ?? "15000",
		10,
	);
...
async function postJson<T>(
	...
	const { response, payload } = await fetchSetupJson<{
		success?: boolean;
		data?: T;
	...
	if (!response.ok || payload.success !== true || payload.data === undefined) {
		throw new SetupApiError(
			payload.error?.message ?? `Auto setup request failed (${response.status})`,
			response.status,
			payload.error?.code,
		);
	}
...
async function acknowledgeSetup(apiUrl: string, deviceCode: string): Promise<void> {
	let lastError: unknown;
	let completionMayHaveSucceeded = false;
	for (let attempt = 1; attempt <= ACKNOWLEDGEMENT_ATTEMPTS; attempt += 1) {
		try {
			await postJson<{ status: "completed" }>(
				apiUrl,
				"/api/auth/mcp-setup/acknowledge",
				{ deviceCode },
			);
			return;
		} catch (error) {
			...
			lastError = error;
			if (attempt < ACKNOWLEDGEMENT_ATTEMPTS) await wait(100);
		}
	}
	throw new SetupAcknowledgementUncertainError(lastError);
}

async function verifyApprovedProfile(
	apiUrl: string,
	apiKey: string,
	profile: SetupProfile,
): Promise<{ toolCount: number; writeCount: number }> {
	...
	const { response, payload } = await fetchSetupJson<{
		success?: boolean;
		data?: { tools?: Array<{ write?: boolean }> };
		error?: { message?: string };
	}>(url, { headers: { "x-auto-api-key": apiKey } });
	if (!response.ok || payload.success !== true || !Array.isArray(payload.data?.tools)) {
		throw new Error(
			payload.error?.message ?? `Profile verification failed (${response.status})`,
		);
	}
	const tools = payload.data.tools;
	if (tools.length === 0) {
		throw new Error(
			`Verification failed: ${profile.name} did not expose any tools`,
		);
	}
	const writeCount = tools.filter((tool) => tool.write === true).length;
	if (profile.accessPreset === "read" && writeCount > 0) {
		throw new Error(
			`Safety verification failed: ${profile.name} exposed ${writeCount} write tool(s)`,
		);
	}
	const expectsWriteTools =
		profile.id === "perps" ||
		profile.id === "trading" ||
	...
	if (profile.accessPreset === "read_write" && expectsWriteTools && writeCount === 0) {
		throw new Error(
			`Verification failed: ${profile.name} did not expose any write tools`,
		);
	}
...
async function resolveInteractiveOptions(options: SetupOptions): Promise<{
	profileName: string;
	preset: McpSurface | "custom";
	clientName: string;
	...
	if ((!profileName || !preset || !clientName) && !stdin.isTTY) {
		throw new Error(
			"Interactive setup needs a terminal. Supply --profile, --preset, and --client for non-interactive use.",
		);
	}
	if (!profileName || !preset || !clientName) {
		...
		try {
				...
				if (!access) {
					const answer =
						(await prompt.question(
							"API access: 1. Read  2. Read + Write [1]: ",
						)).trim() || "1";
					access = answer === "1" ? "read" : answer === "2" ? "read_write" : answer;
				}
				if (!categoriesInput) {
					categoriesInput = await prompt.question(
						"Visible categories (comma-separated, e.g. market-prices,macro): ",
					);
				}
			}
			if (!profileName) {
			...
			if (install === undefined) {
				install = (
					await prompt.question(
						"Install into clients (comma-separated or 'all') [none]: ",
					)
				).trim();
			}
		} finally {
	...
	let accessPreset: AccessPreset | undefined;
	if (preset === "custom") {
		...
		if (categories.length === 0 || invalid.length > 0) {
			throw new Error(
				`Custom setup needs supported categories. Invalid: ${invalid.join(", ") || "none selected"}`,
			);
		}
		if (
			access === "read" &&
			...
			throw new Error(
				"A custom Read profile needs at least one read-capable category",
			);
		}
...
export async function runSetup(options: SetupOptions): Promise<void> {
	...
	const started = await postJson<StartResponse>(
		apiUrl,
		"/api/auth/mcp-setup/authorizations",
		{
	...
	if (
		!Number.isFinite(started.intervalSeconds) ||
	...
	if (options.openBrowser) openExternalUrl(started.verificationUri);
	stdout.write("Waiting for approval…\n");

	let exchange: { apiKey: string; profile: SetupProfile } | null = null;
	while (Date.now() < started.expiresAt) {
		try {
			exchange = await postJson<{ apiKey: string; profile: SetupProfile }>(
				apiUrl,
				"/api/auth/mcp-setup/token",
	...
	if (
		!profileMatchesRequest(exchange.profile, resolved) ||
	...
	const verified = await verifyApprovedProfile(
		apiUrl,
		exchange.apiKey,
		exchange.profile,
	);
	...
	try {
		...
		try {
			...
			throw new AggregateError(
				...
				"Auto setup acknowledgement failed and the previous profile could not be restored",
			);
		}
		throw acknowledgementError;
	}
